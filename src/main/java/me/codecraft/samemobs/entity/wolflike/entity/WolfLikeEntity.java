...
public class WolfLikeEntity extends TameableEntity implements Angerable, IAnimatable {
    ...
    public WolfLikeEntity(EntityType<? extends WolfLikeEntity> entityType, World world) {
    ...
    protected void initGoals() {
        ...
        this.goalSelector.add(1, new WolfLikeEntity.WolfEscapeDangerGoal(1.5));
        this.goalSelector.add(2, new SitGoal(this));
        this.goalSelector.add(3, new WolfLikeEntity.AvoidLlamaGoal(this, LlamaEntity.class, 24.0F, 1.5, 1.5));
    ...
    public WolfLikeEntity createChild(ServerWorld serverWorld, PassiveEntity passiveEntity) {
        WolfLikeEntity WolfLikeEntity = (WolfLikeEntity) SameMobsEntity.WOLFLIKEENTITY.create(serverWorld);
    ...
    public boolean canBreedWith(AnimalEntity other) {
        if (other == this) {
            ...
            return false;
        } else if (!(other instanceof WolfLikeEntity)) {
            return false;
        } else {
            WolfLikeEntity WolfLikeEntity = (WolfLikeEntity)other;
    ...
    public boolean canAttackWithOwner(LivingEntity target, LivingEntity owner) {
        if (!(target instanceof CreeperEntity) && !(target instanceof GhastEntity)) {
            if (target instanceof WolfLikeEntity) {
                WolfLikeEntity WolfLikeEntity = (WolfLikeEntity)target;
    ...
    public static boolean canSpawn(EntityType<WolfLikeEntity> type, WorldAccess world, SpawnReason spawnReason, BlockPos pos, Random random) {
        return world.getBlockState(pos.down()).isIn(BlockTags.WOLVES_SPAWNABLE_ON) && isLightLevelValidForNaturalSpawn(world, pos);
    }

    static {
        BEGGING = DataTracker.registerData(WolfLikeEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
        COLLAR_COLOR = DataTracker.registerData(WolfLikeEntity.class, TrackedDataHandlerRegistry.INTEGER);
        ANGER_TIME = DataTracker.registerData(WolfLikeEntity.class, TrackedDataHandlerRegistry.INTEGER);
    ...
    class WolfEscapeDangerGoal extends EscapeDangerGoal {
        public WolfEscapeDangerGoal(double speed) {
            super(WolfLikeEntity.this, speed);
        }
    ...
    private class AvoidLlamaGoal<T extends LivingEntity> extends FleeEntityGoal<T> {
        private final WolfLikeEntity wolf;

        public AvoidLlamaGoal(WolfLikeEntity wolf, Class<T> fleeFromType, float distance, double slowSpeed, double fastSpeed) {
        ...
        private boolean isScaredOf(LlamaEntity llama) {
            return llama.getStrength() >= WolfLikeEntity.this.random.nextInt(5);
        }

        public void start() {
            WolfLikeEntity.this.setTarget((LivingEntity)null);
            super.start();
        }
